...
jQuery(function () {
    ...
    $("#form-branch").on("submit", function (e) {
        e.preventDefault();
        var fomr = $('form#form-branch')[0];
        var formData = new FormData(fomr);

        if($('#method').val() === 'update'){
            link = laroute.route('admin.branch.update')
        }else{
            link = laroute.route('admin.branch.save')
        }
                ...
                if (response.fail == false) {
                    ...
                    Swal.fire({
                        ...
                        html: `Data Baru Berhasil Disimpan!
                            <br><br>
                            <a href="` + laroute.route('admin.branch') + `" class="btn btn-outline-danger">
                                <i class="si si-close mr-1"></i>Keluar
                            </a> 
                            <a href="` + laroute.route('admin.branch.add') + `" class="btn btn-outline-info">
