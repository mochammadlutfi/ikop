jQuery(function() { 
    ...
    var tujuan = $("#field-tujuan").select2({
        placeholder: 'Pilih Tujuan Kas',
        allowClear: true,
        theme : 'bootstrap4',
        ajax: {
    ...
    $(document).on('click', '#btn-edit', function () {
        ...
        $.ajax({
            url: laroute.route('kas.transfer.edit', { id: id }),
            type: "GET",
            dataType: "JSON",
            beforeSend: function(){
                Swal.fire({
            ...
            success: function(data) {
                ...
                modal.find('#modal_title').text('Ubah Transfer Kas');
                ...
                tujuan_sel_option = new Option(data.tujuan.nama, data.tujuan, true, true);
                tujuan.append(tujuan_sel_option).trigger('change');
    ...
    $('#btn-add').on("click", function(){
        ...
        $('#modal_title').text('Tambah Transfer Kas');
        modal.modal({
            backdrop: 'static',
            keyboard: false
        });
    ...
    $("#form-kas").on("submit", function (e) {
        ...
        {
            url = laroute.route('kas.transfer.update');
        }else{
            url = laroute.route('kas.transfer.store');
        }
...
function load_content(){
    ...
    $.ajax({
        url: laroute.route('kas.transfer'),
        type: "GET",
        dataType: "JSON",
        data: {
            keyword: keyword,
                ...
                $.each(response.data, function(k, v) {
                    $('#data-list tbody').append(`
                        <tr class="c-pointer" onclick="detail('${ response.data[k].kd_trans_kas }')">
                            ...
                            <td>${ response.data[k].tujuan.nama }</td>
...
function detail(id){
    window.document.location = laroute.route('kas.transfer.detail', { id: id });
}