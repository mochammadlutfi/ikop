jQuery(function() { 
    ...
    var induk = $("#field-induk_id").select2({
        placeholder: 'Pilih Induk Klasifikasi',
        allowClear: true,
        theme : 'bootstrap4',
        ajax: {
    ...
    $("#form-klasifikasi").on("submit", function (e) {
        e.preventDefault();
        var fomr = $('form#form-klasifikasi')[0];
        var formData = new FormData(fomr);

        if($("#method").val() == 'update')
        {
            url = laroute.route('akun.klasifikasi.update');
        }else{
            url = laroute.route('akun.klasifikasi.store');
        }
