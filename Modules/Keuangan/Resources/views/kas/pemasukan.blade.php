    ...
    <div class="block block-rounded block-transparent bg-gd-lake">
        <div class="block-content bg-pattern bg-black-op-25">
            <div class="py-20 text-center">
                ...
                <h2 class="h4 font-w400 text-white-op">Transaksi Pemasukan Kas</h2>
            </div>
        </div>
    </div>
    <div class="block">
        <div class="block-header block-header-default">
            <h3 class="block-title">Kelola Data Pemasukan Kas</h3>
        ...
        <div class="block-content pb-15">
            <table class="table table-hover table-striped font-size-sm" id="list-pemasukan">
                <thead>
                    <tr>
                        <th>No Transaksi</th>
                        <th>Tanggal</th>
    ...
    <div class="modal-dialog modal-dialog-popout" role="document">
        <div class="modal-content">
            <div class="block mb-0">
                <div class="block-header block-header-default">
                        <h3 class="block-title modal-title">Tambah Pemasukan Kas</h3>
                ...
                <div class="block-content">
                    <form id="form-pemasukan" onsubmit="return false;">
                        ...
                        <div class="form-group row">
                            <label class="col-lg-4 col-form-label" >Dari Akun</label>
                            <div class="col-lg-8">
...
<script>
$(function () {
    $('#list-pemasukan').DataTable({
        processing: true,
        serverSide: true,
        ordering:false,
        ajax: laroute.route('admin.kas.pemasukan'),
...
jQuery(document).ready(function () {
    ...
    $(document).on('click', '#btn_tambah', function () {
        ...
        $('#modal_title').text('Tambah Pemasukan Kas');
        $('#modal_form').modal({
            backdrop: 'static',
            keyboard: false
        })
    ...
    $("#form-pemasukan").submit(function (e) {
        ...
        if (save_method == 'tambah') {
            link = laroute.route('admin.kas.pemasukan');
            pesan = "Pemasukan Kas Baru Berhasil Ditambahkan";
        } else {
            link = laroute.route('admin.kas.pemasukan');
            pesan = "Pemasukan Kas Berhasil Diperbaharui";
        }
...
function edit(id){
    ...
    $.ajax({
        url : laroute.route('supplier.edit', {id : id}),
        type: "GET",
        dataType: "JSON",
        success: function(response)
        {
            $('#field-provinsi').val(response.provinsi_id);
            $("#field-kota").prop('disabled', false);
            $('#field-kota').html(response.kota);
            $('#field-kota').val(response.kota_id);

            $("#field-kecamatan").prop('disabled', false);
            $('#field-kecamatan').html(response.kecamatan);
            $('#field-kecamatan').val(response.kecamatan_id);


            $("#field-kelurahan").prop('disabled', false);
            $('#field-kelurahan').html(response.kelurahan);
            $('#field-kelurahan').val(response.kelurahan_id);
            ...
            $('#modal_title').text('Perbaharui Data Supplier');
            $('#modal_form').modal({
                backdrop: 'static',
                keyboard: false
            })
