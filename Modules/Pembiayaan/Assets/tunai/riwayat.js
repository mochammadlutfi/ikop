...
function load_content(){
    ...
    $.ajax({
        url: laroute.route('pmb_tunai.riwayat'),
        type: "GET",
        dataType: "JSON",
        data: {
            keyword: keyword,
        ...
        success: function(response) {
            ...
            if(response.data.length !== 0){
                $.each(response.data, function(k, v) {
                    $('#data-list tbody').append(createElementRow(v));
                });
...
function createElementRow(item){
    var row = `<tr>
        ...
        <td>${ item.teller_nama }</td>
        <td class="text-center">
            <a class="btn btn-secondary btn-sm js-tooltip" data-toggle="tooltip" data-placement="top" title="Detail Transaksi" href="${ laroute.route('simkop.invoice', { id:item.no_transaksi }) }">
